...
// Mirror mirrors the package contents to the give registry and git server.
func Mirror(ctx context.Context, c *cluster.Cluster, pkgPaths layout.PackagePaths, filter filters.ComponentFilterStrategy, regInfo types.RegistryInfo, gitInfo types.GitServerInfo, noImgChecksum bool, retries int) error {
	err := pushImagesToRegistry(ctx, c, pkgPaths, filter, regInfo, noImgChecksum, retries)
	if err != nil {
		return err
	}
	err = pushReposToRepository(ctx, c, pkgPaths, filter, gitInfo, retries)
	if err != nil {
		return err
	}
	return nil
